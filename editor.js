document.addEventListener('DOMContentLoaded', () => {
    ...
    const github = {
        async fetch(endpoint, options = {}) {
            ...
            // A 201 (created) or 200 (ok) response has a body, 204 (no content) does not.
    ...
    function handleAuthentication() {
        if (window.location.hash) {
            ...
            if (token) { localStorage.setItem('github_token', token); window.history.replaceState({}, document.title, window.location.pathname); }
        }
    ...
    function logout() { localStorage.removeItem('github_token'); accessToken = null; tinymce.remove(); updateUI(); }
